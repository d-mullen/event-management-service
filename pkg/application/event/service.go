...
func NewService(r event.Repository, eventTSRepo eventts.Repository) Service {
	return &service{eventContext: r, eventTS: eventTSRepo}
}
...
func getEventTSResults(ctx context.Context, results []*event.Event, eventTS eventts.Repository) error {
	...
	log.WithField("occs_with_metadata", occurrencesWithMD).Trace("got results from event-ts-svc")
	if err != nil {
		return err
	}
	...
	if !shouldGetEventTSCounts(req) {
		...
		for _, result := range resp.Results {
			for _, occurrence := range result.Occurrences {
				...
				ival := interval.AtOrAbove(uint64(occurrence.StartTime))
				if occurrence.Status != event.StatusClosed && occurrence.EndTime > 0 { // occurrence is active
					ival = interval.Closed(uint64(occurrence.StartTime), uint64(occurrence.EndTime)) // occurrence is closed
				}
				for _, field := range req.Fields {
					normalizedField := strings.ToLower(field)
					if v, ok := occAsMap[normalizedField]; ok {
						freqMap.Put(ival, map[string][]any{normalizedField: {v}})
					} else if occurrence.Metadata != nil {
						if values, ok2 := occurrence.Metadata[normalizedField]; ok2 {
							freqMap.Put(ival, map[string][]any{normalizedField: values})
						}
					}
				}
			}
...
type countResult map[string]map[string]uint64
